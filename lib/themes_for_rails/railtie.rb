  ...
  class Railtie < ::Rails::Railtie
    ...
    if !Rails.env.development? && !Rails.env.test?
      initializer "themes_for_rails.precompile" do |app|
        ...
        app.config.assets.precompile += Dir["#{Rails.root}/themes/*"].map { |path| "#{path.split('/').last}/all.js" }
        app.config.assets.precompile += Dir["#{Rails.root}/themes/*"].map { |path| "#{path.split('/').last}/all.css" }
      end
